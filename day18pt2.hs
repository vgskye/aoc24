...
djikstra w h walls target visited tovisit
    | MQ.null tovisit = False
    | target `HM.member` visited = True
    | otherwise = do
...
main = do
        ...
        let width = 71
        let height = 71
        let walls = map ((\[a, b] -> (a, b)) . map read . splitOn ",") $ origmap
        let scored n = djikstra width height (HS.fromList $ take n walls) (width - 1, height - 1) HM.empty (MQ.singleton (QueueEntry 0 (0, 0)))
        print (head . map (walls !!) . filter scored $ reverse [0..length walls])
        hClose handle